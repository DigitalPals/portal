...
impl<Message> Binding<Message> {
    ...
    pub fn from_key_press(event: KeyPress) -> Option<Self> {
        ...
        let combination = if is_copy_shortcut(&key, physical_key, modifiers) {
            Some(Self::Copy)
        } else if is_cut_shortcut(&key, physical_key, modifiers) {
            Some(Self::Cut)
        } else if is_paste_shortcut(&key, physical_key, modifiers) {
            Some(Self::Paste)
        } else {
...
fn is_copy_shortcut(
    key: &keyboard::Key<SmolStr>,
    physical_key: keyboard::key::Physical,
    modifiers: keyboard::Modifiers,
) -> bool {
    matches!(key.to_latin(physical_key), Some('c') if modifiers.command())
        || (modifiers.control()
            && !modifiers.shift()
            && !modifiers.alt()
            && matches!(key.as_ref(), keyboard::Key::Named(key::Named::Insert)))
}
...
fn is_cut_shortcut(
    ...
    matches!(key.to_latin(physical_key), Some('x') if modifiers.command())
        || (modifiers.shift()
            && !modifiers.control()
            && !modifiers.alt()
            && matches!(key.as_ref(), keyboard::Key::Named(key::Named::Delete)))
}
...
fn is_paste_shortcut(
    ...
    matches!(key.to_latin(physical_key), Some('v') if modifiers.command() && !modifiers.alt())
        || (modifiers.shift()
            && !modifiers.control()
            && !modifiers.alt()
            && matches!(key.as_ref(), keyboard::Key::Named(key::Named::Insert)))
