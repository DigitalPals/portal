        ...
        S: AsyncWrite + Unpin,
    {
        match self {
            ...
            ClientMsg::SetDesktopSize { width, height } => {
                // SetDesktopSize (TightVNC/TigerVNC extension)
                // +--------------+--------------+---------------------+
                // | No. of bytes | Type [Value] | Description         |
                // +--------------+--------------+---------------------+
                ...
                // | 4            | U32          | flags               |
                // +--------------+--------------+---------------------+
                let mut payload = vec![251u8, 0];
                payload.write_u16(width).await?;
                payload.write_u16(height).await?;
                payload.push(1); // number-of-screens
                payload.push(0); // padding
                payload.write_u32(0).await?; // screen id
                payload.write_u16(0).await?; // x-position
                payload.write_u16(0).await?; // y-position
                payload.write_u16(width).await?;
                payload.write_u16(height).await?;
                payload.write_u32(0).await?; // flags
                writer.write_all(&payload).await?;
                Ok(())
            }
