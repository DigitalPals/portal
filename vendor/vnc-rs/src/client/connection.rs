...
impl VncInner {
    /// Take the output channel receiver out of VncInner.
    /// This allows it to be managed separately from the inner mutex,
    /// preventing recv_event() from blocking input().
    fn output_ch_take(&mut self) -> Receiver<VncEvent> {
        self.output_ch.take().expect("output_ch already taken")
    }
}
...
impl VncInner {
    ...
    // recv_event and poll_event are now handled directly by VncClient
    // using the separated output_ch, avoiding mutex contention with input.
...
pub struct VncClient {
    /// Input channel sender — thread-safe, no mutex needed
    input_ch: Sender<ClientMsg>,
    /// Output channel receiver — protected by its own mutex (separate from input)
    output_ch: Arc<Mutex<Receiver<VncEvent>>>,
    /// Screen dimensions (read-only after construction)
    screen: (u16, u16),
    /// Shared state that needs mutex protection (close/stop channels)
...
impl VncClient {
    ...
    where
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        let mut inner = VncInner::new(stream, shared, pixel_format, encodings).await?;
        let input_ch = inner.input_ch.clone();
        let screen = inner.screen;
        // Take output_ch out of inner — we'll manage it separately
        // We can't move it out since VncInner owns it, so we'll use the
        // inner's output_ch through the mutex only for close/poll, but
        // add a dedicated path for recv/input that avoids mutex contention.
        Ok(Self {
            input_ch,
            output_ch: Arc::new(Mutex::new(inner.output_ch_take())),
            screen,
    ...
    ///
    /// Does NOT acquire the inner mutex — uses the cloned input channel
    /// sender directly, so it never blocks on recv_event().
    pub async fn input(&self, event: X11Event) -> Result<(), VncError> {
        ...
        self.input_ch.send(msg).await.map_err(|_| VncError::ClientNotRunning)?;
        Ok(())
    }
    ...
    ///
    /// Uses a separate mutex from input(), so recv_event() waiting for
    /// data does NOT block input() from sending events to the server.
    pub async fn recv_event(&self) -> Result<VncEvent, VncError> {
        let mut output = self.output_ch.lock().await;
