...
mod tests {
    ...
    #[test]
    fn powerline_separators_fall_back_to_font_renderer() {
        for c in ['\u{E0B0}', '\u{E0B2}', '\u{E0B4}', '\u{E0B6}'] {
            assert!(block_element_rects(c, 0.0, 0.0, 8.0, 16.0).is_none());
            assert!(box_drawing_rects(c, 0.0, 0.0, 8.0, 16.0).is_none());
        }
    }
