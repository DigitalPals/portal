...
pub fn file_viewer_view(
    ...
    let content: Element<'_, Message> = match &state.content {
        ViewerContent::Loading => {
            container(
                text("Loading...").size(16).color(theme.text_muted)
            )
            .width(Fill)
            .height(Fill)
        ...
        ViewerContent::Markdown { content, raw_text, preview_mode } => {
            if *preview_mode {
        ...
        ViewerContent::Image { data, zoom } => {
            image_viewer_view(data, *zoom, state.viewer_id, theme)
        }
        ViewerContent::Pdf { pages, current_page, total_pages } => {
            pdf_viewer_view(pages, *current_page, *total_pages, state.viewer_id, theme)
        }
    };
...
fn file_viewer_toolbar(
    ...
    let preview_toggle: Element<'_, Message> = if matches!(state.file_type, FileType::Markdown) {
        let is_preview = matches!(&state.content, ViewerContent::Markdown { preview_mode: true, .. });
        ...
        button(text(label).size(13).color(theme.text_primary))
            ...
            .padding([6, 12])
            .on_press(Message::FileViewer(FileViewerMessage::MarkdownTogglePreview(viewer_id)))
            .into()
    } else {
...
fn image_viewer_view(
    ...
    // TODO: Use iced's image widget when image feature is properly integrated
    let placeholder = column![
        text("Image Viewer").size(18).color(theme.text_primary),
        Space::new().height(8),
        text(format!("Zoom: {:.0}%", zoom * 100.0)).size(14).color(theme.text_secondary),
        Space::new().height(16),
        row![
            button(text("-").size(16))
                .padding([4, 12])
                .on_press(Message::FileViewer(FileViewerMessage::ImageZoom(viewer_id, zoom - 0.1))),
            Space::new().width(8),
            button(text("+").size(16))
                .padding([4, 12])
                .on_press(Message::FileViewer(FileViewerMessage::ImageZoom(viewer_id, zoom + 0.1))),
        ...
        Space::new().height(8),
        text(format!("Image size: {} bytes", data.len())).size(12).color(theme.text_muted),
    ]
    .align_x(Alignment::Center);
...
fn pdf_viewer_view(
    ...
    // TODO: Implement actual PDF rendering with pdfium-render
    let placeholder = column![
        text("PDF Viewer").size(18).color(theme.text_primary),
        Space::new().height(8),
            ...
            button(text("<").size(16))
                ...
                .on_press_maybe(
                    (current_page > 0).then_some(
                        Message::FileViewer(FileViewerMessage::PdfPageChange(viewer_id, current_page - 1))
                    )
                ),
            Space::new().width(16),
            button(text(">").size(16))
