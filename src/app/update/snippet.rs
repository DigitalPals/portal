...
pub fn handle_snippet(portal: &mut Portal, msg: SnippetMessage) -> Task<Message> {
    match msg {
        ...
        // Execution
        SnippetMessage::Run(snippet_id) => handle_run(portal, snippet_id),
                ...
                if let Some(host_result) = execution.get_host_result_mut(host_id) {
                    ...
                    match result {
                        Ok(success) => {
                            ...
                            host_result.stderr = success.stderr.clone();
                            ...
                            if success.exit_code == 0 {
                                ...
                                let error_msg = if success.stderr.is_empty() {
        ...
        SnippetMessage::ToggleResultExpand(snippet_id, host_id) => {
            ...
            if !toggled
                && let Some(execution) = portal.snippets.executions.get_last_result_mut(snippet_id)
                && let Some(result) = execution.get_host_result_mut(host_id)
            {
                result.expanded = !result.expanded;
            }
            Task::none()
...
fn handle_run(portal: &mut Portal, snippet_id: Uuid) -> Task<Message> {
    ...
    if snippet.host_ids.is_empty() {
        portal
            .toast_manager
            .push(Toast::warning("No hosts assigned to this snippet"));
        return Task::none();
    }
    ...
    if hosts_info.is_empty() {
        portal
            .toast_manager
            .push(Toast::warning("None of the assigned hosts exist"));
        return Task::none();
    }
    ...
    let tasks: Vec<Task<Message>> = hosts_info
        ...
        .map(|(host_id, _host_name, host)| {
            ...
            Task::perform(
                ...
                |(snippet_id, host_id, result, duration_ms)| {
